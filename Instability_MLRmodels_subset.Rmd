...
knitr::opts_chunk$set(echo = FALSE, dpi=300, fig.width = 12, fig.height = 6.5, results="hide", warning=FALSE, message=FALSE)
```
...
lm_grow <- function(x, y, xnames=NULL, plotpreds=TRUE) {
  ...
  while(continue) {
    ...
    if(minrmse >= min(lowest_rmse)) {
      ...
      if(plotpreds) {
        ...
        plot(ypred, y, xlab="LOOCV Predicted V", ylab="True V",
  ...
  if(!is.null(vars_grow)) {
    ...
    plot(predict(lmfinal), y, xlab="Fitted V", ylab="True V")
...
build_rpart <- function(yy, x) {   # CAREFUL: the data.fram dsub is pulled from environment!!!
  ...
  rpart.plot(thepart)
  par(mar=c(4,4,4,2))
  plot(ypred, thedata$yy, xlab="LOOCV predicted V", ylab="true V")
  legend("topleft", legend=paste("R^2 =", round(cor(ypred,thedata$yy)^2, 3)))
  abline(0,1)
  plot(predict(thepart),thedata$yy, xlab="Fitted V", ylab="true V")
...
$RMSE = \sqrt{\frac{1}{n_k}\sum(V_{k[i]}-V^*_{k[i]})^2}$

in which $V^*_{k[i]}$ represents the predicted value for instability score $k$ for culvert $i$, predicted from a prospective model *without* using the data from culvert $i$.  
...
The next row illustrates the predictive power of each iteration of model building, with *LOOCV predicted* $V^*$ on the x-axis, as calculated by models fit *without* each observation, and the $R^2$ reported represents the $R^2$ of *prediction*.  Finally, the plot at the bottom right of this set shows the fitted *V* on the x-axis and true *V* on the y-axis, along with the $R^2$ of model fit.  The line overlayed with these plots is the y=x equality line, not a regression line.
...
* trees treat all numerical variables as categorical, and can perform better if there are nonlinear or threshold effects, but poorly if there are approximately linear effects
* nodes in a tree are conditional on the previous nodes, whereas MLR effects are global.
...
Finally, true *V* vs LOOCV predicted $V^*$ and Fitted *V*, and their respective values of $R^2$.  These can be compared to their MLR counterparts, the final two plots of the MLR set.  Again, the overlayed line is the y=x line, not a regression line.
...
```{r, results='asis', fig.show='hide'}
whichone <- 1

thecolumns <- lm_grow(x=dsub1, y=Vdir1[,whichone], xnames=names(dsub1))

cat("Top model:", "\n","\n",
"V", names(Vdir)[whichone], "~", paste(names(dsub)[thecolumns], collapse=" + "))
```
...
thecolumns <- lm_grow(x=dsub2, y=Vdir2[,whichone], xnames=names(dsub2))
